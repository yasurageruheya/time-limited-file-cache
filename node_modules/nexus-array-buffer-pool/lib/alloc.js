...
parentPort.on("message", /** @param {RequestAlloc} message */message =>
{
	...
	for(let i = requests.length; i--;)
	{
		...
		{
			if(maxByteLength)
				buffer = new ArrayBuffer(Number(byteLength), {maxByteLength: Number(maxByteLength)});
			else
				buffer = new ArrayBuffer(Number(byteLength));

			buffers.push(buffer);
			response.push({id, buffer, byteLength});
		}
		catch (error)
		{
			response.push({id, error, byteLength});
		}
	}
